...
def read_corners_json(path: str) -> Tuple[np.ndarray, str]:
    ...
    if "corners" in data and isinstance(data["corners"], dict):
        ...
        try:
            ...
            raise ValueError(f"Missing corner key: {e}")
        ...
        raise ValueError("JSON does not contain recognizable corner keys.")

    pts = order_corners(pts)

    orig_img_path = data.get("input", "")
    if not isinstance(orig_img_path, str) or not orig_img_path:
        raise ValueError("Corners JSON must include a valid 'input' image path.")

    return pts, orig_img_path
...
def main():
    ...
    ap.add_argument("--corners_json", default= "camera_capture_corners.json",help="Corners JSON used for warping.")
    ap.add_argument("--path_json", default= "solution_path_points.json", help="Path JSON produced by solver.")
    ap.add_argument("--warped-image", default= "maze_warp.png", help="Path to the warped image used when solving (to read W,H). If omitted, tries 'maze_warp.png'.")
    ...
    ap.add_argument("--out-image", default="original_with_path.png", help="Output overlay on the ORIGINAL image.")
    ap.add_argument("--out-json", default="solution_path_points_unwarped.json", help="Output JSON with unwarped points.")
