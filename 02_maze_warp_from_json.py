...
# Transform the maze into a straight, top down rectangular image
# It reads JSON with the corners
# Warps the image into a clean rectangle and saves it
# Warping removes ditortion, better for later processing (grid,solving)
...
#Estimates the warped image size
...
# 4 original maze corners (src) into 4 corners of a perfect rectangle (dst)
...
def main():
    ...
    ok = cv2.imwrite(args.out, warped) #save image
    if not ok:
